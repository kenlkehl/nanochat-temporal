...
- **SFT data:** synthetic conversations generated by a local open-weights LLM, with every candidate run through a strict pre-1985 contamination filter (the same local LLM acts as judge, temperature 0, thinking disabled). UNSAFE/UNSURE samples are dropped.
...
| `nanochat/contamination_filter.py` | Async local-LLM-based pre-1985 contamination judge; reuses the generator's `LocalLLM` (thinking disabled), with regex prefilter and sqlite verdict cache. |
...
- `pyproject.toml` — added `openai`, `lxml`, `aiosqlite`, `python-dotenv`.
...
2. A local OpenAI-compatible LLM endpoint (vLLM, SGLang, llama.cpp server, etc.) for bulk SFT generation *and* contamination filtering — both stages drive the same server. Recommended models (none pinned): google/gemma-4-31B-it (the default in the speedrun), Qwen3-32B-Instruct, Llama-3.3-70B-Instruct, DeepSeek-V3, or Llama-3.1-8B-Instruct for speed. On multi-GPU boxes you can run one vLLM per GPU and the pipeline will load-balance across them; the speedrun can also auto-launch them for you (see "Running vLLM on multiple GPUs" below). No external API key is required.
...
5. Generate bulk SFT data (local LLM only, hours; filter judged by the same server).
6. SFT.
...
- **Filter false negatives** (the dangerous direction): the judge is the same open-weights model that generated the candidate (one pass, temperature 0, thinking disabled). Mitigate with a manual spot-check on ~100 samples before SFT launches, and consider periodically pointing the filter at a stronger model if one is available.
...
- **Pretraining contamination audit:** running the filter over 30B tokens is too expensive. Trust the year metadata; spot-check ~1000 random shard chunks instead.
